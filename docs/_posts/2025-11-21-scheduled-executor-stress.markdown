...
But what if the submitted runnable takes longer to execute than period? This case is covered by the documentation as well, though
it's a bit vague about the exact behaviour:
...
The Javadocs of `ScheduledExecutorService` state:
...
the task `s` that has been scheduled with a delay of `500ms` is not run before `t0, ..., t4` are finished, and therefore runs with
a delay of `~1000ms`, instead of the scheduled `500ms`.
...
I hope that it is easy to see at this point, that at least 5 threads are needed in the above example to get rid of all delays.
